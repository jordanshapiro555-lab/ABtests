<script type="text/javascript">
    (function() {
        ...
        function getDisplayProgram(programName) {
            ...
            if (n === "first steps" || n === "first-steps" || n === "firststeps") return {
                key: "first-steps",
                label: "First Steps",
                order: 2
            };
            if (n === "toddlers" || n === "toddler") return {
                key: "toddlers",
                label: "Toddlers",
                order: 3
            };
            if (n === "twos" || n === "two") return {
                key: "twos",
                label: "Twos",
                order: 4
            };
            ...
            if (n === "preschool") return {
                key: "preschool",
                label: "Preschool",
                order: 6
            };
            if (n === "pre-k" || n === "pre k" || n === "prek") return {
                key: "pre-k",
                label: "Pre-K",
                order: 7
            };
        ...
        function getAvailabilityKey(availabilityEl) {
            ...
            if (classText.indexOf("available") > -1 || text === "available") return "available";
            if (classText.indexOf("limited") > -1 || text === "limited") return "limited";
            if (classText.indexOf("upcoming") > -1 || text === "upcoming") return "upcoming";
            return null;
        }

        // Canonical roster — always rendered, in this order
        var PROGRAM_ROSTER = [{
                key: "infant",
                label: "Infant",
                order: 1
        ...
        function collectAvailabilityItems() {
            ...
            // Seed every roster program with the default "inquire" status
            var grouped = {};
            PROGRAM_ROSTER.forEach(function(p) {
                grouped[p.key] = {
                    key: p.key,
                    programName: p.label,
                    statusKey: "inquire",
                    statusLabel: CONFIG.statusLabels.inquire,
                    order: p.order,
                    priority: CONFIG.statusPriority.inquire
                };
            });

            // Overlay real flags from cards on the page
            Array.prototype.forEach.call(cards, function(card) {
                ...
                // Only overlay onto known roster programs; ignore anything off-roster
                if (!grouped[displayProgram.key]) return;

                var availabilityEl = content.querySelector(CONFIG.availabilitySelector);
                var statusKey = getAvailabilityKey(availabilityEl);
                if (!statusKey) return; // no recognized flag → leave as Inquire

                var priority = CONFIG.statusPriority[statusKey] || 0;

                // Higher-priority real status wins over the seeded Inquire (and over a weaker flag)
                if (priority > grouped[displayProgram.key].priority) {
                    grouped[displayProgram.key] = {
                        key: displayProgram.key,
                        programName: displayProgram.label,
                        statusKey: statusKey,
                        statusLabel: CONFIG.statusLabels[statusKey],
        ...
        function getFormUrl() {
            // Primary: derive from current URL path
            var segments = window.location.pathname.split("/").filter(Boolean);
            var schoolsIdx = segments.indexOf("schools");
            if (schoolsIdx > -1 && segments.length >= schoolsIdx + 4) {
                var base = "/" + segments.slice(schoolsIdx, schoolsIdx + 4).join("/");
                return base + "/our-school/goddard-form";
            }

            // Fallback: find a link on the page that already contains the school base path
            var links = document.querySelectorAll('a[href*="/schools/"]');
            for (var i = 0; i < links.length; i++) {
                var path = links[i].getAttribute("href") || "";
                // strip protocol/domain if present
                path = path.replace(/^https?:\/\/[^/]+/, "");
                var parts = path.split("/").filter(Boolean);
                var idx = parts.indexOf("schools");
                if (idx > -1 && parts.length >= idx + 4) {
                    return "/" + parts.slice(idx, idx + 4).join("/") + "/our-school/goddard-form";
                }
            }
        ...
        function createAvailabilityModule(items) {
            ...
            eyebrow.className = "gsi-avail__eyebrow";
            var eyebrowText = document.createElement("p");
            eyebrowText.textContent = "Availability by Program";
            eyebrow.appendChild(eyebrowText);
            var list = document.createElement("div");
            list.className = "gsi-avail__list";
            list.setAttribute("role", "list");
            list.setAttribute("data-gsi-availability-list", "true");
            items.forEach(function(item) {
                var row = document.createElement("div");
                row.className = "gsi-avail__item";
                row.setAttribute("role", "listitem");
                row.setAttribute("data-gsi-availability-item", "true");
                var program = document.createElement("span");
                program.className = "gsi-avail__program";
                program.textContent = item.programName;
                var badge = document.createElement("span");
                badge.className = "gsi-avail__badge gsi-avail__badge--" + item.statusKey;
            ...
            var linkHtml = formUrl ?
                '<a href="' + formUrl + '" class="gsi-school-navigation__button">' +
                '<span class="gsi-school-navigation__button-text">Informations & Tours</span>' +
                '</a>' :
                '';
            disclaimer.innerHTML =
                '<p class="disclaimer-text">Let’s find the right fit together! Reach out or schedule a tour to chat about classroom options.</p>' +
                linkHtml;
        ...
        function renderAvailabilityModule() {
            ...
            if (items.length < CONFIG.minimumVisibleItems) {
            ...
            if (existing && existing.getAttribute("data-gsi-availability-signature") === signature) {
                if (!existing.querySelector(".disclaimer-section")) {
                    var rebuilt = createAvailabilityModule(items);
        ...
        function initHeroModule() {
            ...
            pollTimer = window.setInterval(function() {
                renderAvailabilityModule();
                if (Date.now() - startedAt > HERO_MAX_WAIT_MS) {
            ...
            if (window.MutationObserver && document.body) {
                observer = new MutationObserver(function() {
                    if (Date.now() - startedAt <= HERO_MAX_WAIT_MS || document.getElementById(MODULE_ID)) {
                        scheduleRender();
                    }
                });
        ...
        function getAvailabilityStatusData() {
            ...
            flags.forEach(function(flag) {
                if (flag.classList.contains("available")) data.hasAvailable = true;
                if (flag.classList.contains("limited")) data.hasLimited = true;
                if (flag.classList.contains("upcoming")) data.hasUpcoming = true;
            });

            if (data.hasAvailable) data.statuses.push("available");
            if (data.hasLimited) data.statuses.push("limited");
            if (data.hasUpcoming) data.statuses.push("upcoming");
        ...
        function sendToTarget(data) {
            if (HAS_SENT) return;
            if (!window.adobe || !adobe.target || typeof adobe.target.trackEvent !== "function") return;
            if (!data.statuses.length) return;

            var params = {
                "profile.gsiHasAvailabilityFlag": "true"
            };
            if (data.hasAvailable) params["profile.gsiAvailabilityHasAvailable"] = "true";
            if (data.hasLimited) params["profile.gsiAvailabilityHasLimited"] = "true";
            if (data.hasUpcoming) params["profile.gsiAvailabilityHasUpcoming"] = "true";
        ...
        function checkForFlags() {
            if (HAS_SENT) return;
            if (targetStartedAt === null) targetStartedAt = Date.now();

            var data = getAvailabilityStatusData();
            if (data.statuses.length) {
                sendToTarget(data);
                return;
            }

            if (Date.now() - targetStartedAt < TARGET_MAX_WAIT_MS) {
                window.setTimeout(checkForFlags, TARGET_CHECK_INTERVAL_MS);
            }
        }
        ...
        function initFullStory() {
            var fsInterval = window.setInterval(function() {
                if (window.FS && typeof FS.setVars === "function") {
                    FS.setVars("page", {
                        ab_test_name: "Schools_LP_Classroom_Availability_Flags",
                        ab_variant: "Variation_Schools_LP_Classroom_Availability_Flags_In_Hero"
                    });
